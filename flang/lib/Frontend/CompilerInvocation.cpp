...
bool CompilerInvocation::createFromArgs(
  ...
  // -finit-integer
  if (const auto *arg =
          args.getLastArg(clang::driver::options::OPT_finit_integer_EQ)) {
    uint32_t argValue = std::atoi(arg->getValue());
    invoc.loweringOpts.setIntegerInitVal(argValue);
    invoc.loweringOpts.setIntegerInitDef(1);
  }
