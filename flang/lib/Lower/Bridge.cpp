...
private:
  ...
  void instantiateVar(const Fortran::lower::pft::Variable &var,
    ...
    auto processImplicitAssignment = [&]() -> void {
      ...
      bool isInitIntegerFlagDefined =
          (getLoweringOptions().getIntegerInitDef() == 1);

      /*
       * Process -finit-integer=<32 bit unsigned integer>
       * Create an implicit assignment of form `var = value`,
       * where `value` is an unsigned 32 bit integer,
       * and generically build the assignment.
       */

      if (isInitIntegerFlagDefined &&
          declTy->IsNumeric(Fortran::semantics::TypeCategory::Integer)) {
        ...
        Fortran::parser::Variable variable = Fortran::parser::Variable{
            Fortran::common::Indirection<Fortran::parser::Designator>{
                std::move(designator)}};
        std::string val =
            std::to_string(getLoweringOptions().getIntegerInitVal());
