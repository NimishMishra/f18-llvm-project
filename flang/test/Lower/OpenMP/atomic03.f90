...
program OmpAtomic
    ...
    !$omp atomic relaxed update 
        y = y - 1
    !$omp atomic release update hint(omp_sync_hint_uncontended)
        y = y * 1
    !$omp atomic update 
        y = y / 1
    !$omp atomic hint(omp_sync_hint_none) update 
        a = a .AND. b
    !$omp atomic update 
        b = b .OR. c
    !$omp atomic update hint(omp_sync_hint_contended)
        a = a .EQV. b
    !$omp atomic update 
        c = c .NEQV. a
    !$omp atomic hint(omp_sync_hint_nonspeculative) update
        x = xor(x, y)
    !$omp atomic update
        z = rshift(z, 3)
    !$omp atomic update hint(omp_sync_hint_speculative)
        z = lshift(z, 2)
    !$omp atomic update
        x = max(x, y, 2, 3)
    !$omp atomic update
