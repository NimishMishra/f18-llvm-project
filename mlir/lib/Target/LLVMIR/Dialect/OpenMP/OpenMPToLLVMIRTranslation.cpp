...
llvm::Value *emitTaskReductionGetThdata(
  ...
  llvm::Value *args[] = {
      ThreadID,
      /// tskgrp=nullptr will pick the current taskgroup enclosing the task
      /// which is okay for now
      llvm::ConstantPointerNull::get(llvm::PointerType::get(Context, 0)), Val};
    ...
    if (taskOp.getInReductionSyms()) {
      ...
      for (mlir::Value inReductionVar : taskOp.getInReductionVars()) {
        llvm::Value *Result = emitTaskReductionGetThdata(
            builder, moduleTranslation, ompLoc,
...
static LogicalResult allocAndInitializeTaskReductionVars(
  ...
  llvm::AllocaInst *TaskRed = builder.CreateAlloca(OpaquePtrTy, nullptr, "");
  ...
  // emitTaskRedInitCall(builder, moduleTranslation, ompLoc,
  // arraySize,ArrayAlloca);
  llvm::Value *CallResult = emitTaskRedInitCall(builder, moduleTranslation,
                                                ompLoc, arraySize, ArrayAlloca);
  builder.CreateStore(CallResult, TaskRed);
  return success();
}
